...
const MenuViewDocument = forwardRef(({ getDataFromChild }, ref) => {
    ...
    return (
        <div>
            <div ref={itemClickRef} className="itemClick" onClick={() => setShowMenu(!showMenu)}>
                <Icon icon="charm:menu-kebab" style={{width: '20px', height: '20px', position: 'relative'}}/>
            </div>
            {showMenu &&
                <div className="popup" style={{ height: '', overflow: 'auto'}}>
                    <div ref={menuRef} className="menuViewDocument-wrapper">
                        <div className="actionMenuViewDocument delete" onClick={() => handleClickAction(2)}>
...
export default MenuViewDocument